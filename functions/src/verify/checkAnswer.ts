  ...
  try {
    ...
    const result = await db.runTransaction(async (tx) => {
      ...
      if (attemptsSnap.size >= 3) {
        throw new HttpsError('resource-exhausted', '인증 시도 횟수(3회)를 초과했습니다.');
      }

      // 아이템 조회 + 해시 비교
      const itemDoc = await tx.get(db.doc(`items/${itemId}`));
      if (!itemDoc.exists) {
        throw new HttpsError('not-found', '아이템을 찾을 수 없습니다.');
      }

      const hashHex = itemDoc.get('verificationAHash') as string;
      if (!hashHex) {
        throw new HttpsError('failed-precondition', '인증 정보가 설정되지 않았습니다.');
      }

      const expected = Buffer.from(hashHex, 'hex');
      const actual = sha256(answer);
      const isCorrect = expected.length === actual.length && timingSafeEqual(expected, actual);
